...
module.exports = {
	...
	async execute(message, args, instanceId) {
		...
		try {
			...
			if (member == undefined) {
				message.channel.send("No user is mentioned");
				return;
			}
			...
			if (!role) {
				message.channel.send(
					":worried: Maaf, role Admin tidak ditemukan. Silakan buat role **Admin** terlebih dahulu _(case sensitive)_"
				);
				return;
			}

			await member.roles.add(role).catch(console.error);
			message.channel.send(`Berhasil menjadikan <@!${member.id} sebagai admin`);
		} catch (error) {
			console.log(error);
			message.channel.send(
